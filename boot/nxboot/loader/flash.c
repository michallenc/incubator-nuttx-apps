...
int flash_partition_open(const char *path)
{
  ...
  if (fd < 0)
    {
      fprintf(stderr, "Could not open %s partition: %s\n",
              path, strerror(errno));
      return ERROR;
    }
...
int flash_partition_write(int fd, const void *buf, size_t count, off_t off)
{
  ...
  if (ret < 0)
    {
      fprintf(stderr, "ioctl MTDIOC_GEOMETRY failed: %s\n", strerror(errno));
      return ERROR;
    }
  ...
  if (count + off > size)
    {
      fprintf(stderr, "Trying to write outside of flash area.\n");
      return ERROR;
    }
  ...
  if (nbytes != count)
    {
      fprintf(stderr, "Write to offset %ld failed %s\n",
              off, strerror(errno));
      return ERROR;
    }
...
int flash_partition_read(int fd, void *buf, size_t count, off_t off)
{
  ...
  if (count + off > size)
    {
      fprintf(stderr, "Trying to read outside of flash area.\n");
      return ERROR;
    }
  ...
  if (nbytes != count)
    {
      fprintf(stderr, "Read from offset %ld failed %s\n",
              off, strerror(errno));
      return ERROR;
    }
...
int flash_partition_erase(int fd)
{
  ...
  if (ret < 0)
    {
      fprintf(stderr, "Could not erase the partition: %s\n",
              strerror(errno));
      return ERROR;
    }
...
/****************************************************************************
 * Name: flash_partition_blocksize
 *
 * Description:
...
int flash_partition_blocksize(int fd)
{
  int ret;
  struct mtd_geometry_s geometry;
 ...
 *
 * Returned Value:
 *   Size of the partition on success, -1 on failure.
 *
 ****************************************************************************/

int flash_partition_size(int fd)
{
  int ret;
  struct mtd_geometry_s geometry;
  ...
  return geometry.erasesize * geometry.neraseblocks;
}
